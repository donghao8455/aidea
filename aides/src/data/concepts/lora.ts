...
const lora: ConceptDetail = {
  ...
  difficulty: 3,
  tags: ['微调', '低秩', '高效训练'],
  tooltip: {summary: '给大模型加装「小型技能插件」做轻量化微调，是 AI 领域的重要概念'},
  detail: {
    definition: 'LoRA（Low-Rank Adaptation，缩写 LoRA）是 AI 领域的重要概念之一。给大模型加装「小型技能插件」做轻量化微调。LoRA 在现代大语言模型、深度学习应用中扮演关键角色，是开发者、研究者和爱好者必须了解的基础知识。掌握 LoRA 的核心原理有助于深入理解 AI 系统的设计哲学和实现方法。',
    plainExplanation: '想象一下，给大模型加装「小型技能插件」做轻量化微调。这就是 LoRA 的核心思想。LoRA 不是什么神秘的魔法，而是一套清晰的工程方法 —— 通过精心设计的算法和数学工具，解决 AI 领域的具体问题。学习 LoRA 能帮你理解 AI 系统「为什么这样工作」，以及「怎么改进它」。',
    analogy: '给大模型加装「小型技能插件」做轻量化微调。这个类比能帮助你快速建立对 LoRA 的直觉理解，把抽象的技术概念映射到熟悉的日常生活场景。',
    keyPoints: [
      "LoRA 是 AI 领域的重要概念，对应 Low-Rank Adaptation",
      "核心思想：给大模型加装「小型技能插件」做轻量化微调",
      "主要应用场景：模型部署、推理加速、专业应用",
      "与 Low-Rank Adaptation 相关的核心技术包括 Transformer、神经网络等",
      "在现代大语言模型生态中扮演重要角色"
    ],
    useCases: [
